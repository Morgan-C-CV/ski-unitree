...
def grab_points(m,d):
 ...
 b=m.body('right_ski_segment_2').id;g=d.xpos[b]+d.xmat[b].reshape(3,3)@np.array([-.075,-.042,.010])
 return p,g
...
def solve_cross():
 ...
 desired=[Rotation.from_euler('z',45,degrees=True).as_matrix(),Rotation.from_euler('z',-45,degrees=True).as_matrix()]
 def evaluate(x,return_q=False):
  ...
  res.append(float(d.xmat[ski_ids[0]].reshape(3,3)[:,0]@d.xmat[ski_ids[1]].reshape(3,3)[:,0])*20)
  # Crossing is behind the bindings. The upper right board clears the lower left.
  for i,b in enumerate(ski_ids):
   R=d.xmat[b].reshape(3,3)
   res.extend((R-desired[i]).ravel()*4)
   res.extend((centers[i]-np.array([.22,.13 if i==0 else -.13,-.35 if i==0 else -.27]))*4)
  res.extend((centers[1]-centers[0]-np.array([0,-.26,.08]))*6)
 ...
 out=dict(joints=dict(zip(names,q.tolist())),qpos=d.qpos.tolist(),grab_distance_m=float(np.linalg.norm(p-g)),cross_angle_deg=angle,board_centers=[d.xpos[b].tolist() for b in ski_ids],minimum_collision_distance_m=min([float(c.dist) for c in d.contact[:d.ncon]] or [0]),grab='right palm to upper right ski, behind binding on outer edge; no finger actuation')
